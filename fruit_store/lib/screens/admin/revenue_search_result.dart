...
class _RevenueSearchResultScreenState extends State<RevenueSearchResultScreen> {
  ...
  Future<void> _search() async {
    ...
    try {
      ...
      List<Order> orders = [];
      try {
        final response = await widget.orderService.getByEmail(widget.query);
        orders = response.items;
      } catch (_) {
        // Fall back to customer name search
        try {
          final response = await widget.orderService.getByCustomerName(
            widget.query,
          );
          orders = response.items;
        } catch (_) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('Results: "${widget.query}"')),
      body: _isLoading
